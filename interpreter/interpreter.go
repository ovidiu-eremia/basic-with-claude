...
func (i *Interpreter) handleNextStatement(program *parser.Program, variableName string, currentLineIndex *int) (bool, error) {
	...
	if variableName != "" {
		...
		if forLoop == nil {
			return false, fmt.Errorf("?NEXT WITHOUT FOR ERROR")
		}
	} else {
	...
	if shouldContinue {
		...
		*currentLineIndex = forLoop.AfterForIndex
		return true, nil
	} else {
